...
int flag=0;
map<ADDRINT, string> mp;

void BeginInstruction(UINT32 rnum, UINT32 wnum, ADDRINT ad, UINT32 rn, UINT32 wn, VOID *ip, UINT32 op_code, VOID *opstring)
{
    instrCount++;
    ...
    if(op_code==426 && rn==56 && wn==56) {
        flag++;
        // cout<<mp[ad]<<endl;
        // cout<<(unsigned char)(REG)rn<<" "<<(unsigned char)(REG)wn<<endl;
    }
...
void BeginInstruction2(void* a, UINT32 rnum, UINT32 wnum, ADDRINT ad, UINT32 rn, UINT32 wn, VOID *ip, UINT32 op_code, VOID *opstring)
{
    instrCount++;
    //printf("[%p %u %s ", ip, opcode, (char*)opstring);
    ...
    curr_instr.update=false;
    if(flag==1) {
        cout<<mp[ad]<<endl;
        // cout<<rnum<<" "<<wnum<<endl;
        cout<<"addr "<<(unsigned long long int)a<<endl;
    }
    if(op_code==426 && rn==56 && wn==56) {
        flag++;
        if(flag==3) {
            curr_instr.update=true;
            flag=2;
        }
        // cout<<mp[ad]<<endl;
        // cout<<(unsigned char)(REG)rn<<" "<<(unsigned char)(REG)wn<<endl;
...
VOID Instruction(INS ins, VOID *v)
{
    ...
    if(memOperands==0) INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)BeginInstruction, IARG_UINT32, INS_MaxNumRRegs(ins), IARG_UINT32, INS_MaxNumWRegs(ins), IARG_ADDRINT, INS_Address(ins), IARG_UINT32, INS_RegR(ins, 0), IARG_UINT32, INS_RegW(ins, 0), IARG_INST_PTR, IARG_UINT32, opcode, IARG_END);
    else INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)BeginInstruction2, IARG_MEMORYOP_EA, 0,IARG_UINT32, INS_MaxNumRRegs(ins), IARG_UINT32, INS_MaxNumWRegs(ins), IARG_ADDRINT, INS_Address(ins), IARG_UINT32, INS_RegR(ins, 0), IARG_UINT32, INS_RegW(ins, 0), IARG_INST_PTR, IARG_UINT32, opcode, IARG_END);
    // instrument branch instructions
    if(INS_IsBranch(ins))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)BranchOrNot, IARG_BRANCH_TAKEN, IARG_END);

    mp[INS_Address(ins)]=INS_Disassemble(ins);
